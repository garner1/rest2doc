...
# mkdir "$datadir"/aux_plus "$datadir"/aux_minus
# wd=$PWD                                                                                                                                                     
# cd "$datadir/aux_plus"
# awk '$6 == "+"' ../refExtendedDoc.bed | awk '{print $7 >> $1; close($1)}' -
# parallel "sed 's/$/NNNNNN/' {} > 1line.{}" ::: chr*
...
# echo "Count kmers ..."   
# mkdir "$datadir"/6mer_{plus,minus}           
# rm -f "$datadir"/6mer_*/*                                                                                                                                                                          
# wd=$PWD                                                                                                                                                     
# cd "$datadir/aux_plus"
# parallel "cat {} | tr -d '\n' | $wd/kmers 6 6 |LC_ALL=C grep -v 'N' | awk 'NF == 2' > ../6mer_plus/{}.tsv" ::: 1line.*                                                                            
# cd "$datadir/aux_minus"
# parallel "cat {} | tr -d '\n' | $wd/kmers 6 6 |LC_ALL=C grep -v 'N' | awk 'NF == 2' > ../6mer_minus/{}.tsv" ::: 1line.*                                                                            
# cd $wd
